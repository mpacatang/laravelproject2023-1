<template>
    <Layout>
        <PageHeader :items="breadcrumb_items" :title="$t('edit_coupon')"/>
...
export default defineComponent({
    ...
    methods: {
        async update() {
            ...
            this.clearAllError();
            try {
                const response = await Request.patchAuth(adminAPI.coupons.update(this.id), {
                    ...this.coupon,
                ...
                if (response.success()) {
                    ToastNotification.success(this.$t('coupon_is_edited'));
                    RequestCache.removeSimilar(adminAPI.coupons.get);
    ...
    async mounted() {
        this.setTitle(this.$t('edit_coupon'));
        try {
            const response = await Request.getAuth<IData<ICoupon>>(adminAPI.coupons.show(this.id));
            this.coupon = response.data.data;
            const responseShops = await Request.getAuth<IData<IShop[]>>(adminAPI.shops.get);
