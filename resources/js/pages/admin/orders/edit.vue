                    ...
                    <Card>
                        ...
                        <CardHeader :title="$t('order_details')" icon="shopping_bag" type="msr" class="py-2">
                            <div class="d-flex float-end">
                                ...
                                <router-link :to="{name: 'admin.orders.invoice', params: {'id': id} }">
                                    ...
                                    <tr :class="[{'border-dashed': item.addons!=null && item.addons.length>0}]"
                                        ...
                                        <td>
                                            <NetworkImage :src="item.product.images[0]?.image"
                                                          rounded
                                                          :placeholder-src="productPortraitPlaceholder"
                                                          :width="48"/>
                                        </td>
                                        <td>
...
export default defineComponent({
    ...
    methods: {
        ...
        chatWithCustomer() {
            this.$router.push({name: 'admin.chats.index', query: {user_type: 'customer', id: this.order.customer.id}})
        },
        chatWithDeliveryBoy() {
            this.$router.push({
        ...
        async getOrder() {
            try {
                const response = await Request.getAuth<IData<IOrder>>(adminAPI.orders.show(this.id), {forced: true});
