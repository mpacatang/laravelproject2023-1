<template>
    <Layout>
        ...
        <PageLoading :loading="shops_loading">
            ...
            <PageLoading v-if="shop!=null"  :loading="page_loading">
                <Row>
                    <Col xxl="9">
                        <Card>
                            <CardBody>
                                ...
                                <Row class="mt-3">
                                    <Col lg="12">
                                        <div class="d-flex transparent-scrollbar justify-content-xl-center"
                                             style=" overflow-x: scroll;">

                                            <template v-for="category in categories">
                                                <VItem align-items="center" class="cursor-pointer " display="flex"
                                                       flex="column"
                                                       style="min-width: 100px" @click="onSelectCategory(category)">
                                                    <div :class="[{'show-check': selected_category===category}]"
                                                         class="category-check">
                                                        <NetworkImage :placeholder-src="categoryPlaceholder" :size="60"
                                                                      :src="category.image"
                                                                      circular/>
                                                    </div>
                                                    <div>
                        ...
                        <template v-if="products.length>0">
                            <Row>
                                <Col v-for="product in filter_products" lg="4" sm="6" xl="3" xs="12">
                ...
                <VModal v-model="showing_product_modal" close-btn lg>
                    <div v-if="selected_product" class="d-lg-flex d-grid p-4 align-items-start">
                        <div >
                        ...
                        <div class="ms-lg-4 ms-0 flex-grow-1">
                ...
                <VModal v-model="showing_cart_modal" close-btn lg>
                    <div v-if="selected_cart" class="d-lg-flex d-grid  p-4 align-items-start">
                        <div>
                            <NetworkImage :src="getProductImageUrl(selected_product)" object-fit="contain" rounded
                                          size="300"/>
                            <div class="mt-2">
...
export default defineComponent({
    ...
    mixins: [FormMixin, UtilMixin, DiscountTypeMixin, AdminModuleSelectorMixin],
    data() {
    ...
    methods: {
        onSelectShop(id) {
            if (id != null && this.selected_shop != id) {
        ...
        deleteCart(cart: ICart) {
            ...
            this.showing_cart_modal = false;
        },
        async createCustomer() {
            this.loading = true;
            this.clearAllError();
            try {
                const response = await Request.postAuth<IData<ICustomer>>(adminAPI.customers.create, {
                    ...this.new_customer
                });
                if (response.success()) {
        ...
        async placeOrder() {
            if(this.selected_shop==null){
                ToastNotification.error(this.$t('select_shop'));
                return;
            }
            if (this.carts.length == 0) {
            ...
            try {
                const response = await Request.postAuth<IData<IOrder>>(adminAPI.pos.create, Request.getClearBody(body));
                if (response.success()) {
                    let order: IOrder = response.data.data;
                    ToastNotification.success(this.$t('order_placed'));
                    this.$router.push({name: 'admin.orders.edit', params: {id: order.id}});
                }
            } catch (error) {
                if (Response.is422(error)) {
                    this.errors = error.response.data.errors;
        ...
        async getShopData() {
            const id = this.selected_shop;
            if (id == null)
                return;
            try {
                const categoryResponse = await Request.getAuth<IData<ICategory[]>>(adminAPI.shops.categories(id));
                this.categories = categoryResponse.data.data;
                const response = await Request.getAuth<IData<IProduct>>(adminAPI.shops.products(id));
                this.products = response.data.data;
                this.filter_products = response.data.data;
                const shopResponse = await Request.getAuth<IData<ICustomer>>(adminAPI.shops.show(id));
        ...
        async getCustomers() {
            try {
                const response = await Request.getAuth<IData<ICustomer>>(adminAPI.customers.get, {forced: true});
        ...
        async onChangeAdminModule(module_id) {
            await this.getShops();
            this.onSelectShop(null);
        }
    },
