                            ...
                            <div class="d-flex align-items-center justify-content-end mt-3">
                                <template v-if="!isSent">
                                    ...
                                    <DateTimePicker v-model="notification.schedule_at" :errors="errors"
                                                    :min-date="Date.now()"
                                                    :placeholder="$t('schedule_notification')" name="schedule_at"
                                                    no-label
                                                    no-spacing/>

                                    <div class="ms-2">
                                        <LoadingButton v-if="notification.schedule_at" :loading="loading"
                                                       color="bluish-purple" flexed-icon icon="schedule_send"
                                                       @click="send">
                                            {{ $t('set_schedule') }}
                                        </LoadingButton>
                                ...
                                <template v-else>
                                    <span v-if="this.notification.schedule_at">{{ $t('notification_already_sent_at') }} : <span
                                        class="fw-medium text-primary">{{
                                            getFormattedDateTime(this.notification.schedule_at)
                                        }}</span></span>
                                    <span v-else>{{ $t('notification_already_sent') }}</span>
                                </template>
...
export default defineComponent({
    ...
    computed: {
        ...
        isSent() {
            return this.notification.schedule_at == null || (Date.parse(this.notification.schedule_at) < Date.now());
        }
    },
    methods: {
        addAction() {
        ...
        async send() {
            ...
            this.clearAllError();
            try {
                const response = await Request.patchAuth(adminAPI.manual_notifications.update(this.id), {
                    ...this.notification,
    ...
    async mounted() {
        ...
        try {
            const response = await Request.getAuth<IManualNotification>(adminAPI.manual_notifications.show(this.id));
