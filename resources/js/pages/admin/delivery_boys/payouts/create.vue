            ...
            <Col xl="8" xxl="6">
                <Card :class="[{'item-disabled': getPayableAmount===0}]">
                    ...
                    <CardBody>
                        <Row gap="3">
                            <Col lg="6">
                                <p class="mb-1 text-nowrap"><span class="d-inline-block"
                                                                  style="width: 210px;">{{ $t('pay') }}
                                    <InfoTooltip class="ms-1"
                                                 tooltip="What amount need to pay to delivery"></InfoTooltip>
                                </span> <span
                                    class="fw-medium">{{ getFormattedCurrency(getTotalPayAmount) }}</span></p>
                                <p class="mb-1 text-nowrap"><span class="d-inline-block"
                                                                  style="width: 210px;">{{ $t('take') }}
                                    <InfoTooltip class="ms-1"
                                                 tooltip="What amount take from delivery"></InfoTooltip>

                                </span> <span
                                    class="fw-medium">{{ getFormattedCurrency(getTotalTakeAmount) }}</span></p>

                                <hr class="dashed my-2" style="width: 230px;margin-left: 10px">

                                <div class="mt-1 font-15 fw-medium text-nowrap">
                                    <span class="d-inline-block text-nowrap"
                                     ...
                                     <InfoTooltip class="ms-1"
                                                  tooltip="Calculated amount based on what you take and give to delivery"></InfoTooltip>
                                </span>
                                    <span class="fw-semibold font-16">
                ...
                <Card>
                    ...
                    <Table :data="delivery_boy_revenues" :loading="table_loading" :option="table_header"
                        ...
                        <template #order_id="data">
                            #{{ data.value }}
                            <!--    TODO <router-link :to="{name: 'admin.orders.edit', params: {id:data.value}}">#{{ data.value }}</router-link>-->
                        </template>
    ...
    methods: {
        generateData(data: IDeliveryBoyRevenue[]) {
            ...
            data?.forEach((revenues) => {
                list.push({
                    'ID': revenues.id,
                    "Delivery": revenues.delivery_boy.first_name + " " + revenues.delivery_boy.last_name,
        ...
        async filter() {
            try {
                ...
                let url = adminAPI.delivery_boy_revenues.get + "?delivery_boy_id=" + this.selected_delivery_boy + "&till_date=" + this.till_date;
        ...
        async create() {
            try {
                ...
                if (this.hasAnyError())
                    return;
                this.loading = true;
                const response = await Request.postAuth(adminAPI.delivery_boy_payouts.create, {
                ...
                if (response.success()) {
                    ...
                    RequestCache.removeSimilar(adminAPI.shop_payouts.get);
                    NavigatorService.goBackOrRoute({name: 'admin.delivery_boy_payouts.index'});
                }
            } catch (error) {
                if (Response.is422(error)) {
                    this.errors = error.response.data.errors;
