                                        ...
                                        <div class="mb-2 d-flex justify-content-end">
                                            <div class="chat-width text-end">
                                                <template v-if="message.type==='image'">
                                                    <div class="text-end">
                                                        <NetworkImage :src="message.image" rounded show-window-preview
                                                                      size="128"/>
                                                    </div>
                                                </template>
                                                <template v-else>
...
export default defineComponent({
    ...
    methods: {
        ...
        async messageLoaded(chat: IChat, message: IChatMessage) {
            let url = adminAPI.chats.message_as_seen(chat.id, message.id);
            Chat.messageLoaded(url, message).then();
            return true;
        },
        async updateChat() {
            try {
                const response = await Request.patchAuth<IData<any>>(adminAPI.chats.update(this.selected_chat.id), {
        ...
        async sendMessage() {
            ...
            try {
                const response = await Request.postAuth<IData<IChatMessage>>(adminAPI.chats.messages(this.selected_chat.id), {
        ...
        async getChats() {
            try {
                const response = await Request.getAuth<IData<IChat[]>>(adminAPI.chats.get, {
                    forced: true
                });
        ...
        async getWithChats() {
            ...
            const url = adminAPI.chats.with + "?user_type=" + userType + "&&id=" + id;
            try {
        ...
        async getChatMessages() {
            ...
            let selected_chat = await Chat.loadMoreMessages(this.selected_chat, adminAPI.chats.messages(this.selected_chat.id));
