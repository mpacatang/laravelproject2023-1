                            ...
                            <ul class="nav  nav-pills arrow-navtabs bg-light">
                                <li v-for="(product_option, index) in product.options" :key="product_option.id"
                                    ...
                                    <a :class="['nav-link px-3', {'active': index===0}]"
                                       ...
                                       tabindex="-1">{{ product_option.option_value ?? "-" }}
                                        <InfoTooltip v-if="optionHasLowStock(product_option)"
                                                     :tooltip="$t('low_stock_alert')"
                                                     class="ms-1 text-orange"></InfoTooltip>
                                    </a>
                                </li>
                            ...
                            <div class="tab-content mt-3">
                                <Col v-for="(product_option, index) in product.options"
                                    ...
                                    <Row>
                                        ...
                                        <Col>
                                            <div class="text-end">
                                                <LoadingButton class="me-2" color="secondary" icon="block"
                                                               icon-size="18" variant="soft"  v-if="canChangeStock"
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            new_product_option: {} as IProductOption,
    ...
    methods: {
        ...
        optionHasLowStock:Product.optionHasLowStock,
        async removeAvailability() {
            this.loading = true;
            try {
                const response = await Request.patchAuth<IData<IProduct>>(sellerAPI.products.remove_availability(this.product.id), {});
                if (response.success()) {
        ...
        async onFileRemoved(file: ILocalFile) {
            if (file.uploaded) {
                try {
                    const response = await Request.deleteAuth(sellerAPI.product_images.delete(file.id));
                    if (response.success()) {
        ...
        async updateProduct() {
            ...
            try {
                ...
                const response = await Request.patchAuth(sellerAPI.products.update(this.product.id), body);
        ...
        async updateOption(product_option: IProductOption) {
            try {
                await Request.patchAuth(sellerAPI.product_options.update(product_option.id), product_option);
        ...
        async createOption() {
            try {
                const response = await Request.postAuth<IProductOption>(sellerAPI.product_options.create, {
    ...
    async mounted() {
        ...
        try {
            ...
            if (response.success()) {
                ...
                const productsResponse = await Request.getAuth<IData<IProduct[]>>(sellerAPI.products.get);
