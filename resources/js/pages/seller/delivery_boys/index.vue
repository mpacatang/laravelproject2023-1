        ...
        <Card>
            <Table :data="filter_delivery_boys" :loading="table_loading" :option="table_header">
                <template #avatar="data">
                    <NetworkImage :from-text="data.row.first_name +' '+data.row.last_name" :src="data.row.avatar"
                                  circular
                                  size="44"/>
                </template>
                ...
                <template #shop="data">
                    <template v-if="data.value">
                        <router-link :to="{name: 'seller.shops.edit', params: {id: data.row.shop.id}}">
                            {{ data.value.name }}
                        </router-link>
                    </template>
                ...
                <template #actions="data">
                    <router-link :to="{name: 'seller.delivery_boys.edit',  params: { id: data.row.id}}">
                        <EditActionButton/>
                    </router-link>
                </template>

                <template v-slot:table-actions>
                    <router-link :to="{name: 'seller.delivery_boys.create', }">
                        <CreateButton/>
                    </router-link>
                </template>
...
export default defineComponent({
    ...
    computed: {
        getInactiveDeliveryBoyCount() {
            return this.delivery_boys.filter((delivery_boy) => !delivery_boy.active).length;
        },
        table_header(): ITableOption<IDeliveryBoy> {
            return {
                ...
                onRefresh: ()=> this.getDeliveryBoys(true),
    ...
    methods: {
        ...
        async getDeliveryBoys(forced=false) {
            this.table_loading = true;
            try {
                const response = await Request.getAuth<IData<IDeliveryBoy>>(sellerAPI.delivery_boys.get, {forced: forced});
