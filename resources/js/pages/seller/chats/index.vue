                            ...
                            <div class="ms-2 flex-grow-1">
                                ...
                                <div v-if="getLastMessage(chat)" class="d-flex align-items-center">
                                    ...
                                    <div v-if="getLastMessage(chat)?.type==='image'" class="flex-grow-1">
                                        ...
                                        <span :class="[{'fw-semibold': !lastMessageIsSeen(chat)}]"
                                              class="font-11 ms-1">{{ $t('image') }}</span>
                                    </div>
                                ...
                                <div v-for="(message, index) in selected_chat.messages">
                                    <template v-if="getPinnedDateFromIndex(index)">
                                        <div class="text-center mb-2">
                                            <span class="bg-light p-0-5 px-2 rounded font-13 fw-medium">
...
export default defineComponent({
    ...
    methods: {
        ...
        async messageLoaded(chat: IChat, message: IChatMessage) {
            let url = sellerAPI.chats.message_as_seen(chat.id, message.id);
            Chat.messageLoaded(url, message).then();
            return true;
        },
        async updateChat() {
            try {
                const response = await Request.patchAuth<IData<any>>(sellerAPI.chats.update(this.selected_chat.id), {
        ...
        async sendMessage() {
            ...
            try {
                const response = await Request.postAuth<IData<IChatMessage>>(sellerAPI.chats.messages(this.selected_chat.id), {
        ...
        async getChats() {
            try {
                const response = await Request.getAuth<IData<IChat[]>>(sellerAPI.chats.get, {
                    forced: true
                });
        ...
        async getWithChats() {
            ...
            const url = sellerAPI.chats.with + "?user_type=" + userType + "&&id=" + id;
            try {
        ...
        async getChatMessages() {
            ...
            let selected_chat = await Chat.loadMoreMessages(this.selected_chat, sellerAPI.chats.messages(this.selected_chat.id));
