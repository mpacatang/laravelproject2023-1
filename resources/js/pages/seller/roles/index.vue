        ...
        <Card>
            <Table :data="roles" :loading="table_loading" :option="table_header">
                <template #actions="data">
                    <router-link :to="{name: 'seller.roles.edit', params: {id:data.row.id}}">
                        <EditActionButton/>
                    </router-link>
                </template>
                <template v-slot:table-actions>
                    <router-link :to="{name: 'seller.roles.create' }">
                        <CreateButton/>
                    </router-link>
                </template>
...
import {ISellerRole} from "@js/types/models/seller";
...
export default defineComponent({
    ...
    computed: {
        ...
        table_header(): ITableOption<ISellerRole> {
            return {
                columns: [
                    {
                        label: this.$t('ID'),
    ...
    methods: {
        ...
        async getRoles(forced = false) {
            this.table_loading = true;
            try {
                const response = await Request.getAuth<IData<ISellerRole[]>>(sellerAPI.roles.get, {forced: forced});
